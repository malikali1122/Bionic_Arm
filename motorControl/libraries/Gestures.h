...
void gestClench(Servo (&servoArr)[4], int (&handState)[4]); // pass by reference to allow changes
void gestOpen(Servo (&servoArr)[4], int (&handState)[4]); // hand state is changed after every servo write operation
void gestPinch(Servo (&servoArr)[4], int (&handState)[4]);

/* NOTES on JM's ARM
    Actuator Closed-Open
    Thumb       0-180
    ThumbRot    80-0
    IndexMiddle 0-180
    RingPinky   0-180
...
// GESTURES
void gestClench(Servo (&servoArr)[4], int (&handState)[4]) {
  int finalAngles[4] = {0, 80, 0, 0};
  int deltaAngles[4];
  for (int i = 0; i < 4; i++)
  {
